...
exports.FreyaDispatcher = void 0;
const util_1 = require("./util");
/** Handles parsing messages and running commands from them */
class FreyaDispatcher {
    ...
    shouldHandleMessage(message, oldMessage) {
        ...
        if (message.partial || message.author.bot || message.author.id === this.Client.user.id)
            return false;
    ...
    inhibit(cmdMsg) {
        for (const inhibitor of this.Inhibitors) {
            ...
            if (inhibit) {
                ...
                const valid = typeof inhibit.reason === 'string' && (typeof inhibit.response === 'undefined' ||
    ...
    buildCommandPattern(prefix) {
        let pattern = prefix ? new RegExp(`^(<@!?${this.Client.user.id}>\\s+(?:${util_1.escapeRegex(prefix)}\\s*)?|${util_1.escapeRegex(prefix)}\\s*)([^\\s]+)`, 'i') : new RegExp(`(^<@!?${this.Client.user.id}>\\s+)([^\\s]+)`, 'i');
        this._commandPatterns[prefix] = pattern;
        this.Client.emit('debug', `Built command pattern for prefix "${prefix}": ${pattern}`);
        return pattern;
    }
}
exports.FreyaDispatcher = FreyaDispatcher;
//# sourceMappingURL=FreyaDispatcher.js.map