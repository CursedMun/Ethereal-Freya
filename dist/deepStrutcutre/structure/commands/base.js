...
class Command {
    ...
    hasPermission(message, ownerOverride = true) {
        ...
        if (message.channel.type === 'text' && this.UserPermissions) {
            const missing = (_a = message.channel.permissionsFor(message.author)) === null || _a === void 0 ? void 0 : _a.missing(this.UserPermissions);
        ...
        switch (reason) {
            ...
            case 'clientPermissions': {
                ...
                return message.reply(common_tags_1.oneLine `
					...
					${data.missing.map((perm) => perm.toLowerCase()).join(', ')}
				`);
            }
            case 'throttling': {
    ...
    onError(err, message, args, fromPattern, result) {
        ...
        const ownerList = owners ? owners.map((usr, i) => {
            ...
            return `${or}${discord_js_1.Util.escapeMarkdown(usr.username)}#${usr.discriminator}`;
    ...
    static usage(command, prefix = "", user) {
    ...
    static validateInfo(client, info) {
        ...
        if (info.clientPermissions) {
            ...
            for (const perm of info.clientPermissions) {
                if (!util_1.permissions[perm.toString()])
