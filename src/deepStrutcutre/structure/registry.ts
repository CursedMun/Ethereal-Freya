...
export class FreyaRegistry {
	 ...
	 */
	registerGroup(group: CommandGroup | Function | Object | string, name: string, guarded: boolean) {
	 ...
	 * ]);
	 */
	registerGroups(groups: (string | boolean)[][]) {
	 ...
	 */
	registerCommand(command: Command | Function): FreyaRegistry {
		...
		if (this.Commands.some((cmd: { name: any; aliases: string | any[]; }) => cmd.name === command.name || cmd.aliases.includes(command.name))) {
			throw new Error(`A command with the name/alias "${command.Name}" is already registered.`);
		}
		for (const alias of command.Aliases) {
			if (this.Commands.some((cmd: { name: any; aliases: string | any[]; }) => cmd.name === alias || cmd.aliases.includes(alias))) {
				throw new Error(`A command with the name/alias "${alias}" is already registered.`);
			}
		}
		const group = this.Groups.find((grp: { id: any; }) => grp.id === command.groupID);
		if (!group) throw new Error(`Group "${command.GroupID}" is not registered.`);
		if (group.commands.some((cmd: { memberName: any; }) => cmd.memberName === command.memberName)) {
	 ...
	 * @return {CommandoRegistry}
	 */
	registerCommands(commands: Command[] | Function[], ignoreInvalid = false): FreyaRegistry {
	 ...
	 */
	registerCommandsIn(options: string): FreyaRegistry {
	 ...
	 */
	registerType(type: ArgumentType | Function): FreyaRegistry {
		/* eslint-disable new-cap */
	 ...
	 * @return {CommandoRegistry}
	 */
	registerTypes(types: ArgumentType[] | Function[], ignoreInvalid = false): FreyaRegistry {
	 ...
	 * @return {CommandoRegistry}
	 */
	registerTypesIn(options: string) {
		const obj = require('require-all')(options);
		const types = [];
	...
	registerDefaultTypes(types = {}) {
		...
		if (types.integer) this.registerType(require('./types/int'));
	 ...
	 */
	findGroups(searchString: string, exact = false) {
	...
	findCommands(searchString: string, exact = false, message: FreyaMessage | null = null): Command[] {
		...
		for (const command of matchedCommands) {
			if (command.Name === lcSearch || (command.Aliases && command.Aliases.some((ali: any) => ali === lcSearch))) {
				return [command];
			}
		}
	 ...
	 */
	resolveCommand(command: Command | string | FreyaMessage) {
		if (command instanceof Command) return command;
		if (command instanceof FreyaMessage && command.Command) return command.Command;
	 ...
	 */
	resolveCommandPath(group: string, memberName: string) {
		return path.join(this.CommandsPath!, group, `${memberName}.js`);
	}
}

function groupFilterExact(search: any) {
	return (grp: { id: any; name: string; }) => grp.id === search || grp.name.toLowerCase() === search;
}

function groupFilterInexact(search: any) {
	return (grp: { id: string | any[]; name: string; }) => grp.id.includes(search) || grp.name.toLowerCase().includes(search);
}

function commandFilterExact(search: string) {
	return (cmd: { name: any; aliases: any[]; groupID: any; memberName: any; }) => cmd.name === search ||
		(cmd.aliases && cmd.aliases.some((ali: any) => ali === search)) ||
		`${cmd.groupID}:${cmd.memberName}` === search;
}

function commandFilterInexact(search: string) {
	return (cmd: { name: string | any[]; groupID: any; memberName: any; aliases: any[]; }) => cmd.name.includes(search) ||
		`${cmd.groupID}:${cmd.memberName}` === search ||
		(cmd.aliases && cmd.aliases.some((ali: string | any[]) => ali.includes(search)));
}
