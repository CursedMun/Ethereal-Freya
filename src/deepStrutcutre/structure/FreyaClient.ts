...
import {  FreyaDispatcher } from './FreyaDispatcher';
import { FreyaClientOptions } from './Interfaces/Interfaces';
...
export class FreyaClient extends Client {
	...
	constructor(options: FreyaClientOptions) {
		...
		this.commandPrefix
		this.Registry = new FreyaRegistry(this);
		this.Dispatcher = new FreyaDispatcher(this, this.Registry);
		this.Provider = null;
		this.settings = new GuildSettingsHelper(this, null);
		this._commandPrefix = null;

		// Set up command handling
		const msgErr = (err: Error) => { this.emit('error', err); };
	 ...
	 * @return {boolean}
	 */
	isOwner(user: UserResolvable | null) {
	 ...
	 * @return {Promise<void>}
	 */
	async setProvider(Provider: SettingProvider | Promise<SettingProvider>) {
	...
	static ValidateInfo(options: FreyaClientOptions) {
