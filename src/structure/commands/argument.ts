...
import { ArgumentInfo } from "../Interfaces/Interfaces";
...
export class Argument {
     ...
     */
    async obtain(msg: CommandoMessage, val: string, promptLimit: number = Infinity): Promise<ArgumentResult> {
     ...
     * @private
     */
    async obtainInfinite(msg: CommandoMessage, vals: string[], promptLimit: number = Infinity): Promise<ArgumentResult> { // eslint-disable-line complexity
        const wait = this.wait > 0 && this.wait !== Infinity ? this.wait * 1000 : undefined;
        const results = [];
        const prompts = [];
        const answers = [];
     ...
     */
    validate(val: string, originalMsg: CommandoMessage, currentMsg: CommandoMessage | null = originalMsg): boolean | string | Promise<boolean | string> {
     ...
     * @return {*|Promise<*>}
     */
    parse(val: string, originalMsg: CommandoMessage, currentMsg: CommandoMessage | null = originalMsg): any | Promise<any> {
     ...
     * @return {boolean}
     */
    isEmpty(val: string, originalMsg: CommandoMessage, currentMsg: CommandoMessage | null = originalMsg): boolean {
     ...
     * @private
     */
    static validateInfo(client: CommandoClient, info: ArgumentInfo) { // eslint-disable-line complexity
     ...
     * @private
     */
    static determineType(client: FreyaClient, id?: string): ArgumentType | null {
        if (!id) return null;
